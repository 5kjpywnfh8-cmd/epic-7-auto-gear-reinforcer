...
- A 类 Markdown 提交 `64639eb`、A 类 JSON 提交 `bdde672`、B 类精确忽略提交 `5c21038` 均已推送。
- 当前工作树剩余 `339` 个未跟踪文件、`223321159` 字节；最大单文件 `26483765` 字节。
- C/D 报告扫描到：5 个文件含本地 IPv4 端点模式，4 个文件含绝对路径模式，1 个文件含 PCAP/原始载荷模式，37 个文件含账号/实例字段模式。扫描只记录数量，不在报告中输出具体值。
- 这些文件保持可见、未忽略、未删除。完成 `git status --porcelain` 收口前，用户必须明确选择：精确外传到 `origin`，或授权先移至工作区外的私有归档并生成恢复 manifest；D 类还需明确是否允许保留原件后制作脱敏提交副本。
